...
// github.com/sony/sonyflake  是索尼对Twitter开源的分布式ID生成算法-雪花算法（SnowFlake）改进后的算法。
import (
	"errors"
...
const (
	...
	epoch int64 = 1659263420000 // 这个是我在写epoch这个常量时的时间戳(毫秒)
)
...
// NewSnowWorker 实例化一个工作节点  workerId 为当前节点的id
...
// GetId 生成方法一定要挂载在某个woker下，这样逻辑会比较清晰 指定某个节点生成id
...
// Sonyflake 索尼雪花算法
func (w *Worker) Sonyflake() *sonyflake.Sonyflake {
	flake := sonyflake.NewSonyflake(sonyflake.Settings{})
	return flake
}

func NextID(flake *sonyflake.Sonyflake) uint64 {
	fid, err := flake.NextID()
	if err != nil {
		log.Fatalf("flake.NextID() failed with %s\n", err)
	}
	// Note: this is base16, could shorten by encoding as base62 string
	log.Printf("github.com/sony/sonyflake:   %x\n", fid)
	return fid
}
